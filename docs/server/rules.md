...
#### 影响游戏平衡与玩家体验的 Mod 与作弊行为 (G.4)

使用作弊 Mod、客户端修改、自动化脚本等非常规手段，

- 直接或间接获得额外**服务器奖励（例如服务器货币，服务器经验）**
- 对服务器运营造成负面影响（例如对服务器硬件造成过重负载）
- 对他人游戏体验造成负面影响
...
- 利用原版机制以外的方法规避服务器AFK检测，以获得额外挂机奖励
- 自动化路径脚本（如长时间跑图）持续加载新区块
- 在 PVP 中使用战斗类作弊（自动攻击，杀戮光环等）攻击玩家

部分行为虽然也使用了客户端修改模组或自动化脚本，但通常不满足上述几条违规条件，例如：
...
在使用红石机器前，请评估其可能对服务器造成的性能影响，并关注服务器 TPS (Tick per Second) 与 MSPT (Milliseconds per Tick) 数据。这两项数据可以反映服务器当前的负载水平。
...
?> 该项的判断也与其他多种因素相关，例如 领地自创建以来经过的时间 和 周边区域的可用土地面积 等，因此符合以上条件的领地不一定会视为无意义圈地
...
生存玩法服务器中可能出现的其他失当行为，但不属于上述各类的归于此项。管理团队使用此项作为处理依据时会明确给出原因。
